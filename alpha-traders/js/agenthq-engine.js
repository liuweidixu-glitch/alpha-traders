...
(function () {
  ...
  function churn() {
    ...
    if (ns === "idle") ag.data.task = "空闲 · standby";
    if (ns === "waiting") ag.data.task = "等待下一项任务";
    if (ns === "working" && /空闲|等待|error|standby/i.test(ag.data.task))
      ag.data.task = "开始新任务 · 正在处理";
    if (ns === "error") ag.data.task = "发生错误 · 需要检查";
  ...
  function renderPopup(ag) {
    ...
    popup.querySelector(".pp-today").textContent = (d.tasksToday ?? 0) + " 今日任务";
    // avatar swatch
