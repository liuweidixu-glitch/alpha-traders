...
class OrderBlockAgent extends BaseAgent {
  ...
  analyze(data) {
    ...
    this.report = {
      ...
      action: near ? (near.type==='bull'?'🟢 在 demand — 等待 buy':'🔴 在 supply — 等待 sell') : '— 等待价格进入区域',
    };
...
class SweepAgent extends BaseAgent {
  ...
  analyze(data) {
    ...
    this.report = {
      ...
      sweep: bullSweep ? '🟢 Bullish sweep (扫低后反弹)' : bearSweep ? '🔴 Bearish sweep (扫高后回落)' : '— 无 sweep',
    };
...
class FVGAgent extends BaseAgent {
  ...
  analyze(data) {
    ...
    this.report = {
      ...
      action:  score>20?'🟢 在 demand 区域':score<-20?'🔴 在 supply 区域':'— 等待价格进入缺口',
    };
...
/* ═══════════════════════════════════════════════════════
   ...
   所有条件必须同时满足 5 层才会触发 (conf 95)
   1) News clear  2) Liquidity Sweep  3) Discount/Premium
   4) OB + FVG trigger  5) Macro DXY 不反向
   设计目的是"基金考试": 少发、精准、低回撤
   ═══════════════════════════════════════════════════════ */
class PropFirmSniperAgent extends BaseAgent {
  ...
  analyze(data) {
    ...
    if (!candles || candles.length < 55) return blank('数据不足 (<55 根K线)');

    const last  = candles.at(-1);
    const price = last.close;

    // ── 第 1 层: NEWS — 如果时间窗口内有重大新闻则暂停交易 (NewsAgent 在 HIGH risk 时返回 watch) ──
    const news        = this._news.analyze();
    const newsBlocked = news.signal === 'watch';

    // ── 第 2 层: LIQUIDITY SWEEP — 扫过 20 根K线的 swing 后反弹 ──
    const look    = candles.slice(-20, -1);
    const swingHi = Math.max(...look.map(c => c.high));
    const swingLo = Math.min(...look.map(c => c.low));
    const bullSweep = last.low  < swingLo && last.close > swingLo;   // 扫低 → 反弹
    const bearSweep = last.high > swingHi && last.close < swingHi;   // 扫高 → 回落

    // ── 第 3 层: PREMIUM / DISCOUNT — 在 50 根K线 range 中的位置 ──
    const range = candles.slice(-50);
    const hi = Math.max(...range.map(c => c.high));
    const lo = Math.min(...range.map(c => c.low));
    const rangePos   = (hi > lo) ? (price - lo) / (hi - lo) : 0.5;   // 0=便宜 1=昂贵
    const isDiscount = rangePos <= 0.40;
    const isPremium  = rangePos >= 0.60;

    // ── 第 4 层: ENTRY TRIGGER — Order Block + Fair Value Gap ──
    const obs  = TA.orderBlocks(candles);
    const fvgs = TA.fvg(candles);
    ...
    // ── 第 5 层: MACRO DXY — USD 偏弱(≤0)→buy, USD 偏强(≥0)→sell (从全局读取) ──
    ...
    let signal='wait', conf=30, verdict='— 等待满足 5 层条件';
    if (buyConfluence)        { signal='buy';  conf=95; verdict='🟢 SNIPE BUY — confluence 满 5/5'; }
    else if (sellConfluence)  { signal='sell'; conf=95; verdict='🔴 SNIPE SELL — confluence 满 5/5'; }
    else if (newsBlocked)     { signal='wait'; conf=25; verdict='⛔ 暂停交易 — 有重大新闻 (risk HIGH)'; }
    else if (buyHits>=4 && buyHits>=sellHits)  { signal='watch'; conf=45; verdict=`🔭 准备买入 BUY (${buyHits}/5)`; }
    else if (sellHits>=4)     { signal='watch'; conf=45; verdict=`🔭 准备卖出 SELL (${sellHits}/5)`; }

    this.signal = signal; this.conf = conf;
    this.report = {
      verdict,
      news:  newsBlocked ? '⛔ HIGH risk — 暂停' : '✓ clear',
      sweep: bullSweep ? '🟢 bull sweep' : bearSweep ? '🔴 bear sweep' : '— 无',
      zone:  `${(rangePos*100).toFixed(0)}% ${isDiscount?'DISCOUNT 🟢':isPremium?'PREMIUM 🔴':'EQ ⚪'}`,
      entry: nearBullOB ? `bull OB @${nearBullOB.origin.toFixed(d)}${hasBullFVG?' +FVG✓':' (等待 FVG)'}`
           : nearBearOB ? `bear OB @${nearBearOB.origin.toFixed(d)}${hasBearFVG?' +FVG✓':' (等待 FVG)'}`
           : '— 价格未进入 OB',
      dxy:   dxyTrend>0?`▲ USD 偏强 (${dxyTrend.toFixed(2)})`:dxyTrend<0?`▼ USD 偏弱 (${dxyTrend.toFixed(2)})`:'↔ flat',
...
class PatternAgent extends BaseAgent {
  ...
  analyze(data) {
    ...
    // Hammer — STRICT: lower wick > 3x body, must be at bottom
      ...
      contextMsg = ' (在 recent low — reversal candidate)';
    }
    // Shooting Star — STRICT: upper wick > 3x body, must be at top
      ...
      contextMsg = ' (在 recent high — reversal candidate)';
    }
    ...
    // 调整阈值: 现在 score ≥ 20 才会触发 actionable signal
...
/* ═══════════════════════════════════════════════════════
   DIVERGENCE ANALYST — RSI + MACD divergence (转折点)
   捕捉 regular + hidden divergence
     - Bull div: 价格创新低但指标未创新低 → 反转向上
     - Bear div: 价格创新高但指标未创新高 → 反转向下
   ═══════════════════════════════════════════════════════ */
...
class IchimokuAgent extends BaseAgent {
  ...
  analyze(data) {
    ...
    if (!candles || candles.length < 78) {
      return { signal:'wait', conf:30, report:{ note:'需要 78+ 根K线' }, log:'Insufficient data' };
    }

    // ── Skip if Scalp M1 mode (Ichimoku slow indicator, M1 = noise) ──
    if (typeof Settings !== 'undefined' && Settings.get('tradeMode', 'swing') === 'scalp') {
      return { signal:'wait', conf:40, report:{ note:'⏸ 不在 Scalp Mode 中使用 (slow indicator)' }, log:'Skipped — Scalp mode' };
    }
...
class Commander {
  ...
  decide(goldReport, currReport) {
    ...
    // ── ADX gate: 在横盘市场 (ADX 低) 降低 confidence + 禁止 buy/sell signal ──
    ...
    // Position size calculator — 根据账户 + risk% 计算仓位
    ...
    const riskWarning = idealLot < 0.01
      ? `⚠️ Risk ${actualRiskPct}% 超过目标 ${riskPct}% — 使用 cent account 或更小的 TF`
      : null;
